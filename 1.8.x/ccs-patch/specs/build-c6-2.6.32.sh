...
if [ ! -r kernel-2.6.32-754.28.1.el6.src.rpm ]
then
    wget http://vault.centos.org/centos/6/updates/Source/SPackages/kernel-2.6.32-754.28.1.el6.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-2.6.32-754.28.1.el6.src.rpm | grep -F ': rsa sha1 (md5) pgp md5 OK' || die "Can't verify signature."
rpm -ivh kernel-2.6.32-754.28.1.el6.src.rpm || die "Can't install source package."
...
+%define buildid _tomoyo_1.8.6p2
 
 %define distro_build 754.28.1
...
+# TOMOYO Linux
+tar -zxf %_sourcedir/ccs-patch-1.8.6-20200413.tar.gz
