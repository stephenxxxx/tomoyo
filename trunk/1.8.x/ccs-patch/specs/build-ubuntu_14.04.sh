#! /bin/sh
#
# This is kernel build script for ubuntu 14.04's 3.13 kernel.
#
...
if [ ! -r ccs-patch-1.8.9-20220621.tar.gz ]
then
    wget -O ccs-patch-1.8.9-20220621.tar.gz 'https://osdn.jp/frs/redir.php?f=/tomoyo/49684/ccs-patch-1.8.9-20220621.tar.gz' || die "Can't download patch."
fi
...
apt-get source linux-source-3.13.0 || die "Can't install kernel source."
for i in `awk ' { if ( $1 != "Build-Depends:") next; $1 = ""; n = split($0, a, ","); for (i = 1; i <= n; i++) { split(a[i], b, " "); print b[1]; } } ' linux-3.13.0/debian/control`; do apt-get -y install $i; done

# Apply patches and create kernel config.
cd linux-3.13.0/ || die "Can't chdir to linux-3.13.0/ ."
update_maintainer
tar -zxf ~/rpmbuild/SOURCES/ccs-patch-1.8.9-20220621.tar.gz || die "Can't extract patch."
patch -p1 < patches/ccs-patch-3.13-ubuntu-14.04.diff || die "Can't apply patch."
