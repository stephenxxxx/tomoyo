...
then
    wget https://vault.centos.org/3.9/updates/SRPMS/kernel-2.4.21-63.EL.src.rpm || die "Can't download source package."
fi
LANG=C rpm --checksig kernel-2.4.21-63.EL.src.rpm | grep -F ': (sha1) dsa sha1 md5 gpg OK' || die "Can't verify signature."
...
+%define release 63.EL_tomoyo_1.8.9
 %define sublevel 21
