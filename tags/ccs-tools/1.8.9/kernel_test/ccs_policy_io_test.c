...
static void stage_policy_io_test(void)
{
	...
	policy_fp = domain_fp;
	for (i = 0; i < 3; i++) {
		try_io("file execute /bin/true", 1);
		try_io("file execute /bin/true exec.realpath=\"/usr/bin/true\"", 1);
		try_io("file execute /bin/true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true keep exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true initialize exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true reset exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true child exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true parent exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true /bin/true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true bin/true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true bin.true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 0);
		try_io("file execute /bin/true usr/bin.true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true usr.bin/true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 0);
		try_io("file execute /bin/true <true> exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true <true> /true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true <true> true/false exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("file execute /bin/true <true> /true /false /true exec.realpath=\"/usr/bin/true\" exec.argv[0]=\"true\"", 1);
		try_io("ipc signal 9 <kernel>", 1);
		try_io("ipc signal 9 <kernel> /bin/true", 1);
		try_io("ipc signal 9 <kernel> bin/true", 1);
		try_io("ipc signal 9 <kernel> usr/bin.true", 1);
		try_io("ipc signal 9 <kernel> usr.bin/true", 0);
		try_io("ipc signal 9 <kernel> usr/bin.true auto_domain_transition=\"<true>\"", 1);
		try_io("ipc signal 9 <kernel> usr/bin.true auto_domain_transition=\"true.false\"", 0);
		try_io("ipc signal 9 <kernel> usr/bin.true auto_domain_transition=\"<true> /bin/false\"", 0);
		try_io("ipc signal 9 <kernel> usr/bin.true auto_domain_transition=\"/bin/true\"", 1);
		try_io("ipc signal 9 <kernel> usr/bin.true 0=0/1 auto_domain_transition=\"<true>\"", 1);
		try_io("ipc signal 9 <kernel> usr/bin.true 0=0/1 auto_domain_transition=\"true.false\"", 0);
		try_io("ipc signal 9 <kernel> usr/bin.true 0=0/1 auto_domain_transition=\"<true> /bin/false\"", 0);
		try_io("ipc signal 9 <kernel> usr/bin.true 0=0/1 auto_domain_transition=\"/bin/true\"", 1);
